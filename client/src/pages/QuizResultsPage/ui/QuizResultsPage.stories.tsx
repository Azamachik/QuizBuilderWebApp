...
const baseAttempt = {
    id: 'a1',
    quizId: '1',
    quizTitle: 'UX/UI Best Practices Quiz',
    inviteLinkToken: 'demo-token',
    label: 'Общая ссылка',
        ...
        StoreDecorator(
            {
                ...
                inviteLink: { isLoading: true } as never,
                attempt: { isLoading: true, isSubmitting: false, sessionQuestions: [] } as never
            },
...
export const GoodResult: Story = {
    ...
    decorators: [
        StoreDecorator(
            {
                user: { authData: undefined, _inited: true },
                inviteLink: { session: { inviteLink: mockLink, quiz: mockQuiz, questions: mockQuestions }, isLoading: false } as never,
                attempt: {
                    currentAttempt: {
                        ...baseAttempt,
                        answers: [
                            { questionId: 'q1', selectedOptionIds: ['a'] },
                            { questionId: 'q2', selectedOptionIds: [], textAnswer: 'Figma 3 года' }
                        ],
                        score: 1,
                        total: 1
                    },
        ...
        StoreDecorator(
            {
                user: { authData: undefined, _inited: true },
                inviteLink: {
                    session: { inviteLink: { ...mockLink, maxUses: 5, usedCount: 5 }, quiz: mockQuiz, questions: mockQuestions },
                    isLoading: false
                } as never,
                attempt: {
                    currentAttempt: { ...baseAttempt, answers: [{ questionId: 'q1', selectedOptionIds: ['a'] }], score: 1, total: 1 },
                    isLoading: false,
                    isSubmitting: false,
                    sessionQuestions: mockQuestions
                } as never
...
export const NotFound: Story = {
    decorators: [
        StoreDecorator(
            {
                user: { authData: undefined, _inited: true },
                inviteLink: { isLoading: false } as never,
                attempt: { isLoading: false, isSubmitting: false, sessionQuestions: [] } as never
            },
            { inviteLink: inviteLinkReducer, attempt: attemptReducer }
        )
    ]
